...
- インターネット接続
- 管理者権限不要（ユーザー領域にインストール）

---

### 1) Miniforge をインストール（初回のみ）
以下の公式ページから **Miniforge3** をインストールしてください。

https://conda-forge.org/download/

- インストール先は既定のままでOK  
  （例：`/Users/＜ユーザー名＞/miniforge3`）
- インストール途中で表示される  
  **「Proceed with initialization? [yes|no]」には `yes` を推奨**  
  （後続のセットアップが簡単になります）

---

### 2) セットアップ（ダブルクリック）
Finder で以下を **ダブルクリック**します。

```
Setup Lilot.command
```

- 初回は macOS のセキュリティ警告が出る場合があります  
  → **右クリック → 開く → 開く**
- ターミナルが起動し、環境構築が自動で進みます
- 「セットアップ完了」と表示されたら閉じてください

---

### 3) 起動（ダブルクリック）
セットアップ完了後は、以下を **ダブルクリックするだけ**で起動できます。

```
Run Lilot.command
```

- ブラウザが自動で開き、以下にアクセスします  
  http://localhost:8501
...
- conda 環境の削除：
```bash
conda remove -n lilot --all
```
- lilot フォルダを削除すれば、ローカルデータもすべて削除されます

---

### macOS 補足
- `.command` ファイルは **プレーンテキスト**である必要があります  
  （TextEdit で作成する場合は「標準テキスト」に変換してください）
- 初回のみ Gatekeeper により実行がブロックされる場合があります  
  → Finder で **右クリック → 開く**
